...
for (const file of listCertificateFiles(CERT_DIR)) {
  ...
  if (output !== source) {
    for (const name of names) {
      const finalConsonant = hasFinalConsonant(name);
      if (finalConsonant === null) continue;
      const wrongParticles = finalConsonant ? ["는", "가", "를", "와"] : ["은", "이", "을", "과"];
...
const sitemapSource = `import fs from "node:fs";
import path from "node:path";
...
console.log(`Estimated particle occurrences corrected: ${correctedOccurrences}`);
