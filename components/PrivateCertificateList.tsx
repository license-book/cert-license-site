...
export default function PrivateCertificateList({ items, popularNames }: Props) {
    ...
    <>
      <section id="certificate-search" className="scroll-mt-24 border-b border-slate-200 bg-white">
        <div className="mx-auto max-w-[1200px] px-5 py-10 md:px-6 md:py-14">
          <div className="mb-6">
            <p className="text-sm font-black tracking-[0.14em] text-violet-600">ALL CERTIFICATES</p>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-slate-50 p-5 shadow-sm md:p-7">
            <label htmlFor="private-cert-search" className="sr-only">
              민간자격증 검색
            </label>
            <div className="relative">
              ...
              <input
                id="private-cert-search"
                type="search"
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                placeholder="자격증명, 분야, 운영기관으로 검색해 보세요"
                className="h-14 w-full rounded-2xl border border-slate-300 bg-white pl-14 pr-5 text-[15px] font-semibold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-violet-500 focus:ring-4 focus:ring-violet-100 md:h-16 md:text-base"
              />
            </div>

            {selectedCategory ? (
              <div className="mt-5 flex flex-wrap items-center gap-3 rounded-2xl border border-violet-200 bg-violet-50 px-4 py-3">
                <span className="text-sm font-black text-violet-800">선택 분야: {selectedCategory}</span>
                <button
                  type="button"
                  onClick={clearCategory}
                  className="text-sm font-black text-violet-600 underline underline-offset-4 hover:text-violet-800"
                >
                  전체 분야 보기
                </button>
              </div>
              ...
              {INITIALS.map((initial) => {
                ...
                return enabled ? (
                  <a
                    ...
                    className="inline-flex h-10 min-w-10 items-center justify-center rounded-xl border border-violet-200 bg-white px-3 text-sm font-black text-violet-700 transition hover:border-violet-500 hover:bg-violet-50"
                  >
                    {initial}
                  </a>
                ) : (
      ...
      <CertificateHubSections
        items={items}
        basePath="/private-certificates"
        popularTitle="인기 민간자격증"
        categoryTitle="분야별 민간자격증"
        theme="violet"
        ...
        <div className="mb-8 flex items-end justify-between gap-4">
          <div>
            ...
            <p className="mt-2 text-sm font-semibold text-slate-500 md:text-base">
              검색 결과 <strong className="text-violet-600">{filteredItems.length}개</strong>
            </p>
          </div>
          {query || selectedCategory ? (
            <button
              ...
              className="shrink-0 text-sm font-bold text-slate-500 underline underline-offset-4 hover:text-violet-600"
            >
              검색·필터 초기화
            </button>
          ) : null}
            ...
            {availableInitials.map((initial, groupIndex) => {
              ...
              return (
                <Fragment key={initial}>
                  ...
                  aria-labelledby={`private-heading-${initial}`}
                >
                  <div className="mb-5 flex items-center gap-4 border-b border-slate-200 pb-4">
                    <h3
                      id={`private-heading-${initial}`}
                      className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-600 text-xl font-black text-white shadow-sm"
                    >
                      {initial}
                    </h3>
                  ...
                  <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
                    {group.map((item) => (
                      <Link
                        key={item.slug}
                        href={`/cert/${item.slug}`}
                        className="group flex min-h-36 flex-col justify-between rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition duration-200 hover:-translate-y-0.5 hover:border-violet-300 hover:shadow-md"
                      >
                        <div>
                          ...
                          <h4 className="text-lg font-black tracking-tight text-slate-950 transition group-hover:text-violet-600">
                            {item.name}
                          </h4>
                          <p className="mt-2 line-clamp-1 text-sm font-semibold text-slate-500">{item.agency}</p>
                        </div>

                        <span className="mt-5 inline-flex items-center gap-1 text-sm font-black text-violet-600">
                          상세정보 보기
